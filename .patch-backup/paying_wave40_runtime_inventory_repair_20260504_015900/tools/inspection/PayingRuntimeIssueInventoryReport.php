...
function paying_command(string $root, string $suite): string
{
    ...
    $parts[] = '--testsuite';
    $parts[] = $suite;
    $parts[] = '--display-warnings';
    $parts[] = '--display-notices';
    $parts[] = '--display-deprecations';
    $parts[] = '--display-skipped';
    $parts[] = '--no-progress';

    return implode(' ', array_map('paying_shell_arg', $parts));
}

function paying_run_suite(string $root, string $suite): array
{
    $output = [];
    $exitCode = 0;
    exec(paying_command($root, $suite) . ' 2>&1', $output, $exitCode);
...
$suites = ['unit', 'functional', 'security'];
$results = [];
...
foreach ($suites as $suite) {
    $result = paying_run_suite($root, $suite);
    $lines = $result['output'];
...
foreach ($results as $result) {
    ...
    if ($result['issues'] !== []) {
        echo "Issue lines:\n";
        foreach (array_slice($result['issues'], 0, 40) as $issue) {
            echo '  L' . $issue['line'] . ': ' . $issue['text'] . "\n";
        }

        if (count($result['issues']) > 40) {
            echo '  ... ' . (count($result['issues']) - 40) . " more issue lines omitted\n";
        }
    } else {
