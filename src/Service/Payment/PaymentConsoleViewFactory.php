...
class PaymentConsoleViewFactory
{
    /**
     * @param array<string, mixed> $consoleView
     */
    public function create(array $consoleView, string $query, string $status, string $selectedPaymentId): PaymentConsoleViewPayload
    {
        ...
        return new PaymentConsoleViewPayload(
            PaymentConsoleViewPayload::WORD,
            PaymentConsoleViewPayload::VIEW_CONSOLE,
    ...
    private function buildTemplateName(string $view, string $template): string
    {
        return sprintf('%s/%s.%s', $view, $template, implode('.', ['html', 'twig']));
    }
}
