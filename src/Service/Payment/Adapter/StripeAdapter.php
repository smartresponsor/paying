...
class StripeAdapter implements GatewayPortInterface
{
    ...
    public function start(Payment $payment, array $context = []): array
    {
        ...
        // Normally create PaymentIntent via Stripe SDK using STRIPE_SECRET
        return ['provider' => 'stripe', 'status' => 'processing', 'id' => (string)$payment->id()];
    }
    ...
    public function reconcile(Ulid $id): Payment
    {
        $p = $this->repo->findById($id);
        if (!$p) throw new \RuntimeException('Payment not found');
        // Normally fetch intent status; here we keep as-is
        return $p;
    }
}
