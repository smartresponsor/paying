 ...
 */
final class PaymentResponseHeaderSubscriberTest extends TestCase
{
    public function testOnResponseAddsHeadersToMainRequest(): void
    {
        $subscriber = new PaymentResponseHeaderSubscriber("default-src 'self'; frame-ancestors 'none'");
    ...
    {
        $subscriber = new PaymentResponseHeaderSubscriber("default-src 'self'");
        $response = new Response();
        $event = new ResponseEvent(
            $this->createMock(HttpKernelInterface::class),
            new Request(),
